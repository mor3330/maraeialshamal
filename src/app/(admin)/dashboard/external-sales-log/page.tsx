...
export default function ExternalSalesLogPage() {
  ...
  const totalPrice  = sales.reduce((s, p) => s + toN(p.price),    0);
  const totalQty    = sales.reduce((s, p) => s + toN(p.quantity), 0);
  ...
  sales.forEach(p => {
    ...
    byItemType[name].count++;
    byItemType[name].weight += toN(p.weight);
    byItemType[name].price  += toN(p.price);
  });
  ...
  return (
    <div className="min-h-screen bg-bg text-cream p-6">
      <div className="max-w-7xl mx-auto">
        ...
        {/* العنوان */}
        <div className="mb-8">
          <h1 className="text-4xl font-black mb-2">سجل المبيعات الخارجية</h1>
          <p className="text-muted">عرض وتحليل جميع المبيعات الخارجية حسب الفترة الزمنية</p>
        </div>
        ...
        <div className="bg-card rounded-3xl border border-line p-6 mb-6">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            ...
            <div>
              <label className="text-xs text-muted block mb-1">الصنف</label>
          ...
          <div className="mt-4 flex gap-3 items-center">
            ...
            <span className="text-sm text-muted">
              النتائج: <span className="text-green font-bold">{sales.length}</span> عملية
            </span>
          </div>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-3 mb-6">
          <div className="rounded-3xl border border-line bg-card p-6">
            <p className="text-muted text-sm mb-2">إجمالي العمليات</p>
            <p className="text-4xl font-black text-green">{sales.length}</p>
          </div>
          <div className="rounded-3xl border border-line bg-card p-6">
            <p className="text-muted text-sm mb-2">إجمالي الوزن</p>
            <p className="text-4xl font-black text-blue-400 ltr-num" dir="ltr">{fmtN(totalWeight)} <span className="text-lg">كجم</span></p>
          </div>
          <div className="rounded-3xl border border-line bg-card p-6">
            <p className="text-muted text-sm mb-2">إجمالي المبلغ</p>
            <p className="text-4xl font-black text-amber-400 ltr-num" dir="ltr">{fmtN(totalPrice)} <span className="text-lg">ر</span></p>
          </div>
        </div>
        ...
        <div className="bg-card rounded-3xl border border-line overflow-hidden">
          <div className="px-6 py-4 border-b border-line">
            <h2 className="text-lg font-bold">تفاصيل المبيعات الخارجية</h2>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-16">
              <p className="text-muted text-lg">لا توجد مبيعات خارجية في هذه الفترة</p>
            </div>
          ) : (
                ...
                <tbody>
                  {sales.map((s, i) => (
                    <tr key={s.id} className={`border-b border-line/50 hover:bg-card-hi transition-colors ${i%2===0?"":"bg-bg/30"}`}>
                      ...
                      <td className="p-3 font-bold ltr-num" dir="ltr">{s.quantity}</td>
                      <td className="p-3 text-blue-400 font-bold ltr-num" dir="ltr">{fmtN(toN(s.weight))} <span className="text-xs text-muted">كجم</span></td>
                      <td className="p-3 text-amber-400 font-bold ltr-num" dir="ltr">{fmtN(toN(s.price))} <span className="text-xs text-muted">ر</span></td>
                      <td className="p-3 text-xs text-muted">{s.notes || "-"}</td>
                    </tr>
                  ))}
                </tbody>
