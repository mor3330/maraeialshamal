...
export async function GET() {
  ...
  const [branchesRes, reportsRes, purchasesRes, itemTypesRes] = await Promise.all([
    ...
    supabase.from("purchases")
      .select("item_type_id, purchase_date, weight, price")
      .gte("purchase_date", twoDaysAgo),
    supabase.from("item_types").select("id, name, name_en").eq("is_active", true),
  ]);
  ...
  const purchasesByDate: Record<string, Record<"hashi"|"sheep"|"beef", { weight: number; price: number }>> = {};
  ...
  const enrichedReports = (reportsRes.data ?? []).map((report: any) => {
    ...
    // بيانات اللحوم
    let meatData = null;
    if (notesObj?.step1Named && notesObj?.step3Named) {
      ...
      meatData = {
        ...
        sold: {
          hashi: { bone_weight: toN(s3.hashi_bone_weight), bone_price: toN(s3.hashi_bone_price), clean_weight: toN(s3.hashi_clean_weight), clean_price: toN(s3.hashi_clean_price) },
          sheep: { weight: toN(s3.sheep_weight), price: toN(s3.sheep_price) },
          beef:  { bone_weight: toN(s3.beef_bone_weight), bone_price: toN(s3.beef_bone_price), clean_weight: toN(s3.beef_clean_weight), clean_price: toN(s3.beef_clean_price) },
        },
